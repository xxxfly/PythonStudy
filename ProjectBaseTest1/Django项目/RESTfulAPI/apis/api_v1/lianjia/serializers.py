...
class GetCJPageListSerializer(serializers.Serializer):
    ...
    search_condition = serializers.ListField()
    selected_condition = serializers.ListField()
    ...
    def get_list(self, validated_data):
        ...
        # 获取数据
        try:
            page_size = validated_data['pageSize']  # 分页大小
            page_index = validated_data['pageIndex']  # 当前页码
            search_condition = validated_data['searchCondition']  # 搜索条件列表
            selected_condition = validated_data['selectedCondition']  # 选择条件列表

            searchName = search_condition[0]

            whlianjiachengjiao_list = whlianjiachengjiao.objects.all().order_by('dealDate')
            if searchName != '' and searchName != None:
                whlianjiachengjiao_list = whlianjiachengjiao_list.filter(title_contains=searchName)
            ...
            result["datalist"] = list(whlianjiachengjiao_list)
