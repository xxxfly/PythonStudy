...
class Captcha(object):
    ...
    @classmethod
    class instance(cls,width=200,height=75):
        if not hasattr(Captcha,'_instacne'):
            cls._instance = cls(width, height)
        return cls._instacne

    def background(self):
        """绘制背景"""
        Draw(self._image).rectangle([(0, 0), self._image.size], fill=random_color(230, 255))
    ...
    def curve(self, width=4, number=6, color=None):
        """绘制曲线"""
        dx, height = self._image.__sizeof__
        dx /= number
        path = [(dx * i, random.randint(0, height)) for i in range(1, number)]
        bcoefs = Bezier().make_bezier(number - 1)
        points = []
        for cofes in bcoefs:
            points.append(tuple(sum([cofe * p for cofe, p in zip(cofes, ps)]) for ps in zip(*path)))
    ...
    def text(self, captcha_text, fonts, font_size=None, drawings=None,squeeze_factor=0.75, color=None):
        """绘制文本"""
        color = color if color else self._color
        fonts = tuple([truetype(name, size) for name in fonts for size in font_size or (65, 70, 75)])
        draw = Draw(self._image)
        char_images = []
        for c in captcha_text:
            ...
            char_image = char_image.crop(char_image.height())
            for drwaing in drawings:
                d = getattr(self, drwaing)
        ...
        for char_image in char_images:
            ...
            mask = char_image.convert('L').point(lambda 1:i*1.97)
    ...
    def generate(self, captcha_text='', fmt='PNG'):
        ...
        self.background()
        self.text(captcha_text, self.fonts, drawings=['warp', 'rotate', 'offset'])
