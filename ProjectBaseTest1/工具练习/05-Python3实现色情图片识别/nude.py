...
class Nude(object):
    ...
    #使用 collections.namedtuple() 定义一个 Skin 类型
    Skin=namedtuple("Skin",'id skin region x y')
    def __init__(self,path_or_image):
        #若path_or_image 为 Image.Image 类型的实例，则直接赋值
        if isinstance(path_or_image,Image.Image):
            self.image=path_or_image
        #若 path_or_image 为 str 类型的实例，则打开图片
        elif isinstance(path_or_image,str):
            self.image=Image.open(path_or_image)
        
        #获得图片所有颜色通道
        bands=self.image.getbands()
        #判断是否为单通道图片（也即 灰度图），是则将灰度图转换为 RGB 图
        if len(bands)==1:
            #新建相同大小的 RGB 图
            new_img=Image.new('RGB',self.image.size)
            #拷贝灰度图 self.image 到 RGB 图 new_image.paste (PIL 自动进行颜色通道转换)
        ...
        self.merge_regions=[]
        #整合后的皮肤区域，元素的所有即为皮肤区域号，元素都是包含一些 skin 对象的列表
        self.skin_regions=[]
    ...
    def resize(self,maxwidth=1000,maxheight=1000):
        ...
        if maxheight:
            if self.height>maxheight:
                ...
                #size – 包含宽高像素数的元祖 (width, height) resample – 可选的重采样滤波器 返回 Image 对象
    ...
    def parse(self):
        ...
        if self.result is not None:
            return self
        #获取图片所有像素的数据
        piexls=self.image.load()

        #遍历每个像素，为每个像素创建对应的 Skin 对象
        for y in range(self.height):
            for x in range(self.width):
                #得到像素的 RGB 三个通道的值
                #[x,y]是 [(x,y)] 的简便写法
                r=piexls[x,y][0] #red
                g=piexls[x,y][1] #green
                b=piexls[x,y][2] #blue
                # 判断当前像素是否为肤色像素
                ...
                #为每个元素创建一个对应 Skin 对象，并添加到 self.skin_map 中
                ...
                check_indexes=[_id-2,#当前像素左方元素
                               _id-self.width-2,#当前元素左上方的像素
                               _id-self.width-1,#当前元素上方的像素
                               _id-self.width #当前元素右上方的元素
                ]
                ...
                for index in check_indexes:
                    ...
                    if self.skin_map[index].skin:
                        ...
                        if (self.skin_map[index].region!=None and region!=None and region!=-1 and self.skin_map[index]!=region and self.last_from!=region and self.last_to!=self.skin_map[index].region):
                            #那么添加这两个区域的合并任务
                ...
                if region==-1:
                    ...
                    _skin=self.skin_map[_id-1]._replace(region=len(self.detected_regions)) #somenamedtuple._replace(kwargs) 返回一个替换指定字段的值为参数的 namedtuple 实例
